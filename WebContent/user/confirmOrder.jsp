...
<form name="order" method="post" action="user/orderSave">
	...
	<table cellpadding=3 cellspacing=1 align=center class=tableborder1
		...
		<c:forEach items="${shopcart.lines }" var="entry">
			<tr>
				<td class=tablebody2 valign=middle align=center width="">${entry.value.book.id
					}</td>
				<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a
					href="user/productDetail?id=${entry.value.book.id}" target="_blank">${entry.value.book.name
						}</a></td>
				<td class=tablebody2 valign=middle align=center width="">价格：${entry.value.book.price
					}</td>
				<td class=tablebody1 valign=middle align=center width="">数量：${entry.value.num
					}</td>
				<td class=tablebody2 valign=middle align=left width="">小计：￥${entry.value.book.price
					* entry.value.num }</td>
			</tr>
		</c:forEach>
		<tr>
	...
	<p align="center">
		请认真检查以上订单信息，确认无误后，点击 → <a
			onclick="javascript:document.order.submit();" style="cursor: pointer"><img
